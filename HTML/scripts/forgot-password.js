...
(function($){
    ...
    //             $.ajax({
    //                 url: "http://"+SERVER_IP+"/user/forgot/password",
    //                 type: "POST",
    //                 dataType: "json",
    //                 data: {
    //                     email
    ...
    function lateSync(){
        $('#sendMail').click(()=>{
            ...
            $.ajax({
                ...
                error: function (data) {
                    // alert('error');
                   $(".notify").text(data.responseJSON['message'] || "Failed to send mail.Try again later!");
                   $('.notify')[0].style.color="red";
                    window.popup(data.responseJSON['message'] || 'Failed to send mail.Try again later!',"errorPopup");
                    console.log(data);
                },
                success: function (data) { //callback   
                    // console.log('Verified successfull');
